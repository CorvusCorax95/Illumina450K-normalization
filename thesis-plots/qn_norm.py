...
fig.suptitle("Quantile Normalizations")

ax[0].set_title("C) Type 1 beta-value densities",
                   loc='left')
ax[0].set_xlabel("beta-value")

ax[1].set_title("D) Type 2 beta-value densities",
                   loc='left')
ax[1].set_xlabel("beta-value")


sample_list = df_qn_beta_t1.columns.values.tolist()[1:]

for sample in sample_list:
	sns.kdeplot(df_qn_beta_t1[sample], color=light_blue, ax=ax[0])
	sns.kdeplot(df_qn_beta_t2[sample], color=light_purple, ax=ax[1])

sns.kdeplot(df_qn_beta_t1['Median'], color=dark_blue, ax=ax[0])
sns.kdeplot(df_qn_beta_t2['Median'], color=dark_purple, ax=ax[1])
